...
def get_skating_events(calendar_id):
    """Retrieve skating events (Public Skating and Adult Open Skate) from the public calendar."""
    ...
    for component in gcal.walk():
        if component.name == "VEVENT":
            ...
            if summary and ("public skating" in summary.lower() or "adult open skate" in summary.lower()):
                events.append({
                    'summary': summary,
...
def event_exists(service, calendar_id, event):
    ...
    for existing_event in events:
        if 'summary' in existing_event and (
            "public skating" in existing_event['summary'].lower() or
            "adult open skate" in existing_event['summary'].lower()
        ):
            return True
    return False
